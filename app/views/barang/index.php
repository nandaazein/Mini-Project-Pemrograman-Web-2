 ...
 <div class="row mb-3">
    ...
    <table class="table table-bordered">
        ...
        <?php $no = 1; ?>
        <?php foreach($data['barang'] as $pgd) : ?>
            <tr>
                <td><?= $no ?></td>
                <td><?= $pgd['nama'] ?></td>
                <td><?= $pgd['id_transaksi'] ?></td>
                <td><?= $pgd['berat_barang'] ?> gram</td>
                <td><?= $pgd['nilai_taksiran'] ?></td>
                <td>
                  <a href="<?= base_url; ?>/barang/edit/<?= $pgd['id_barang_gadai'] ?>" class="badge badge-info ">Edit</a> <a href="<?= base_url; ?>/barang/hapus/<?= $pgd['id_barang_gadai'] ?>" class="badge badge-danger" onclick="return confirm('Hapus data?');">Hapus</a>
                </td>
            </tr>
        <?php $no++;?>
        <?php endforeach; ?>
